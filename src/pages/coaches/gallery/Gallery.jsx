      ...
      <main className="max-w-7xl mx-auto py-6 px-4 lg:px-8">
        ...
        >
          <div className="overflow-x-auto">
            <div className="mb-6">
              <h2 className="text-xl font-semibold text-gray-100">Gallery Class List</h2>
              <p className="text-xs mt-1 text-gray-400">Pick a class to add photos for students</p>
            </div>
            {classes.length > 0 ? (
              <ul className="space-y-6">
                {classes.map((classItem) => (
                  <li key={classItem.classID} className="flex flex-col items-start p-4 border rounded-lg shadow-sm bg-gray-700 space-y-4 md:space-y-0 md:flex-row md:items-center md:justify-between">
                    ...
                    <Link to={`/coach/add-photos/${classItem.classID}`} className="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600 transition-all duration-300">
                      Add Photos
                    </Link>
                  </li>
                ))}
